...
<div class="card-body">
    @empty($user)
        ...
        <div class="form-group row">
            <label class="col-1 control-lavel col-form-label">Level</label>
            <div class="col-11">
                <select class="form-control" name="level_id" id="level_id" required>
                    <option value="">- Pilih Level -</option>
                    @foreach ($level as $item)
                        <option value="{{ $item->level_id }}" @if ($item->level_id == $user->level_id) selected                            
                        @endif>{{ $item->level_id }}</option>   
                    @endforeach
                </select>
                @error('level_id')
